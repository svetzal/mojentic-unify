...
Status: **✅ Phase 7 (v0.7.0 + v0.7.1) shipped (2026-05-18)** — documentation polish, 1.x stabilization, and the full release pipeline. 0.7.0: Dokka v2 multi-module HTML across all six library modules, four handwritten Use Case guides under `docs/use-cases/` (Building Chatbots, Structured Output, Building Agents, Image Analysis), binary-compatibility-validator baselined (`./gradlew apiDump` → six `*.api` files; `apiCheck` wired into the gate), Android Compose chat sample under `samples/android-compose-chat/`, iOS SPM + XCFramework smoke recipe under `samples/ios-spm-smoke/`. 0.7.1: XCFramework binary declarations on every library module (`assembleMojentic<Name>XCFramework` tasks), Maven Central publishing via `com.vanniktech.maven.publish` 0.30.0 targeting the Sonatype Central Portal with in-memory GPG signing, OWASP `dependencyCheckAggregate` at root (CVSS≥7 fails the build), and three GitHub Actions workflows (`docs.yml` → GitHub Pages on `v*` tags, `release.yml` → Maven Central publish on `v*` tags, `security.yml` → weekly NVD scan). All four 0.7.0-deferred infrastructure items now landed; only repo-admin one-time actions remain (enable Pages, add CI secrets, push `v1.4.0`). Phase 6 (Anthropic gateway via Ktor Client) shipped 2026-05-18 — complete / completeJson (via forced `respond_in_json` tool) / stream (SSE with `text_delta`, `thinking_delta`, `input_json_delta` accumulation) / model listing; tool calls round-trip through `tool_use` / `tool_result` content blocks; multimodal user messages encode base64 images; system messages flow through the top-level `system` field; `reasoning_effort` is filtered with a warning until Anthropic accepts it. Phase 5 (`mojentic-realtime-openai` + `mojentic-core` realtime layer) shipped 2026-05-18 — OpenAI Realtime gateway over Ktor WebSockets, vendor-neutral `RealtimeVoiceBroker` / `RealtimeSession` / `RealtimeEvent` / `ClientRealtimeEvent` sealed unions, `AudioFrame`, `VadConfig`, `RealtimeVoiceConfig`, server + manual VAD, barge-in via coroutine cancellation, parallel tool execution per voice turn (reusing Phase 3's `ParallelToolRunner`), end-to-end text-mode example. Phase 4 (`Event`/`Agent`/`Router`/`AsyncDispatcher`/`ToolWrapper`, `SharedWorkingMemory`, `BaseAsyncLlmAgentWithMemory`, `AsyncAggregatorAgent`, `IterativeProblemSolver`, `SimpleRecursiveAgent`, `ReActAgent` + nine examples) shipped 2026-05-18. Phase 3 (Tracer, ParallelToolRunner, AskUser/TellUser, EphemeralTaskList + seven task tools, FilesystemGateway + eight file tools, WebSearch + SerpApi) shipped 2026-05-18; Phase 2 (`ChatSession`, `mojentic-openai`, embeddings, tokenizer, image analysis) shipped 2026-05-18; Phase 1 (core LLM + broker + Ollama gateway) shipped the same day. Quality gate green on JVM + Android-host + iOS-simulator.
...
### Phase 5 — Realtime Voice ✅ Shipped (2026-05-18)
- ✅ `mojentic-realtime-openai` module with `OpenAiRealtimeGateway` over Ktor Client WebSockets (OkHttp engine on JVM/Android, Darwin engine on iOS). Sends initial `session.update` on open; raw server events flow through a `Channel`-backed `Flow<JsonObject>` so no events are dropped between connect and the broker's first subscription.
- ✅ Core vendor-neutral surface: `RealtimeGateway` / `RealtimeGatewaySession` interfaces, `RealtimeVoiceBroker`, `RealtimeSession`, `RealtimeVoiceConfig` (cross-port subset of OpenAI's session config plus a `providerExtras` escape hatch), `RealtimeEvent` sealed union (~20 variants covering session, user turn, assistant turn, parallel-aware tool calls, control), `ClientRealtimeEvent` sealed union for everything the client sends, `AudioFrame` PCM16 carrier type, `VadConfig.Server` / `VadConfig.Manual` sealed types.
- ✅ Pcm16AudioCodec in `mojentic-core/realtime/internal` so the broker can decode audio deltas without depending on the OpenAI module.
- ✅ Server VAD (default) + manual VAD with `session.commit()` for push-to-talk; `sendAudio(Flow<AudioFrame>)` streams PCM frames into the session; barge-in cancels the active tool-dispatch `Job` + sends `ResponseCancel` + emits `Interrupted(reason = BargeIn)` — all driven by `UserSpeechStarted` arriving mid-turn.
- ✅ Tool dispatch lives in its own `Job` owned by the session so barge-in can abort it cleanly. Tools reuse the existing `LlmTool` interface and run through `ParallelToolRunner` by default — voice turns benefit from concurrent tool execution.
- ✅ `OpenAiEventCodec` is the only place in the stack that knows OpenAI's wire vocabulary; the broker speaks the vendor-neutral types end-to-end. `session.rawEvents: Flow<JsonObject>` is the power-user / debugging escape hatch, re-published by the broker so multiple subscribers can coexist.
- ✅ Example: `examples/realtime-text` — JVM-only smoke runner exercising text modality end-to-end against the live OpenAI Realtime endpoint (requires `OPENAI_API_KEY`).
- ✅ Tests: 11 codec tests + 8 session tests + 6 audio/VAD/barge-in tests using a `FakeRealtimeGateway` that replays scripted wire events through a Channel-backed flow. Quality gate: ktlint + Detekt clean, `./gradlew build allTests` green across JVM + Android-host + iOS-simulator.

### Phase 6 — Anthropic Gateway ✅ Shipped (2026-05-18)
- ✅ `mojentic-anthropic` module: `AnthropicGateway` matching the Python feature set. Ktor Client, Anthropic Messages API at `/v1/messages`, `x-api-key` + `anthropic-version` headers, `host` constructor knob for Bedrock-compatible deployments.
- ✅ Full `LlmGateway` surface: `complete()`, `completeJson()`, `stream()`, `availableModels()`.
- ✅ Message adaptation (`AdaptedMessages.kt`): system messages flow through the top-level `system` field (Anthropic does not accept system messages inside the `messages` array); user/assistant/tool messages translate to `text` / `image` / `tool_use` / `tool_result` content blocks; multimodal images encoded as `image/{mime}` base64 sources.
- ✅ Structured output: `completeJson` wraps the caller's schema in a synthetic `respond_in_json` tool with `tool_choice = { type: "tool", name: "respond_in_json" }`, returns the parsed tool input. Throws `LlmGatewayException` if the model declines to call the forced tool.
- ✅ SSE streaming: `AnthropicStreamAccumulator` handles `message_start`, `content_block_start`, `content_block_delta` (`text_delta` / `thinking_delta` / `input_json_delta`), `content_block_stop`, `message_delta`, `message_stop`, `ping`, `error`. Tool-call arguments accumulate across `input_json_delta` chunks until the matching `content_block_stop`, then emit as one `GatewayStreamEvent.ToolCalls`.
- ✅ Matches Python's quirks: `reasoning_effort` is filtered out with a warning (Anthropic does not yet accept it on the wire); `max_tokens` falls back to `DEFAULT_MAX_TOKENS = 4096` when `CompletionConfig.maxTokens <= 0`.
- ✅ Example: `examples/anthropic-simple` — JVM-only smoke runner against the live Anthropic endpoint (requires `ANTHROPIC_API_KEY`).
- ✅ Tests: 12 gateway tests + 3 streaming tests with Ktor `MockEngine`, run on JVM + Android-host + iOS-simulator. Quality gate: ktlint + Detekt clean, `./gradlew build allTests` green.

### Phase 7 — Documentation polish & 1.x stabilization ✅ Shipped (2026-05-18)
- ✅ Dokka v2 multi-module HTML aggregation at root (`./gradlew dokkaGenerate` → `build/dokka/html/`). Per-library-module `org.jetbrains.dokka` plugin application; root `dependencies { dokka(project(":…")) }` aggregates all six. V2 enabled via `gradle.properties` opt-in flag.
- ✅ Four handwritten Use Case guides under `mojentic-kt/docs/use-cases/`: Building Chatbots, Structured Output, Building Agents, Image Analysis. `docs/index.md` is the landing page included via `dokkaPublications.html { includes.from(...) }`.
- ✅ Binary-compatibility validator (`kotlinx.binary-compatibility-validator` 0.18.1) applied at root with `apiValidation { ignoredProjects = [all examples], nonPublicMarkers += "com.mojentic.internal.InternalApi" }`. Initial baselines committed for `mojentic-core/api/jvm/mojentic-core.api` (1798 LOC) and the five gateway modules. `./gradlew apiCheck` wired into the quality gate.
- ✅ Sample Android Compose app at `samples/android-compose-chat/` — `ChatViewModel` + `ChatScreen` + `MainActivity` + Manifest + strings.xml, with a README covering copy-paste integration into a freshly-generated Android Studio Empty-Compose-Activity project. Deliberately not a separate Gradle module (the sample is documentation, not library distribution; building it would require adding Compose plugin + Compose BOM + ~6 androidx artefacts to the library's dep tree).
- ✅ iOS SPM + XCFramework consumption smoke recipe at `samples/ios-spm-smoke/README.md` — XCFramework task declaration template, `Package.swift` with `binaryTarget(path:)`, single-file Swift main exercising `LlmMessage`, `xcodebuild -destination 'platform=iOS Simulator'` step. Documented procedure; CI automation under `macos-latest` is the followup once Maven Central publishing lands.
- ✅ Maven Central publishing pipeline (`com.vanniktech.maven.publish` 0.30.0 + Sonatype Central Portal + in-memory GPG signing) wired per library module; full POM metadata; `publishAndReleaseToMavenCentral` task runs in `.github/workflows/release.yml` on `v*` tags (0.7.1).
- ✅ GitHub Pages deployment of `dokkaGenerate` output on every `v*` tag via `.github/workflows/docs.yml` using the standard `actions/deploy-pages` flow (0.7.1).
- ✅ OWASP `dependencyCheckAggregate` task applied at root with `failBuildOnCVSS = 7.0`, `dependency-check-suppressions.xml` baseline (empty), NVD API key read from env, and a weekly `.github/workflows/security.yml` schedule (0.7.1).
- ✅ XCFramework declarations on every library module (`assembleMojentic<Name>XCFramework` tasks) — release + debug variants for the three iOS targets (0.7.1).
- ✅ Quality gate green: `./gradlew ktlintCheck detekt build allTests apiCheck` → BUILD SUCCESSFUL, 509 actionable tasks across JVM + Android-host + iOS-simulator.
