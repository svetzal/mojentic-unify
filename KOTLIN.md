...
- 📝 Maven Central publishing pipeline (`gradle-maven-publish-plugin` + Sonatype OSSRH credentials) — code is ready, secrets must be wired into CI by repo admins; deferred to `0.7.1`.
- 📝 GitHub Pages deployment of `dokkaGenerate` output on every `v*` tag — pages enablement is a one-time repo-admin action; deferred to `0.7.1`.
- 📝 OWASP `dependencyCheckAggregate` — plugin declared in `libs.versions.toml` but not yet applied; deferred to `0.7.1` with an explicit suppressions file to avoid the initial-CVE triage thrash on a release-candidate baseline.
